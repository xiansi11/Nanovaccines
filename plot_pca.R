######## ######## ######## ######## ######## 
######## PCA analysis to combine the four responses 
...
trts = c('Saline',
         ...
         'MPLA nanovax',  'R848 nanovax',"Flu shot")
...
data = dat0 %>%
  mutate(sum.score = Bronchiolitis.bronchitis+ 
           ...
           Hemorrhage+
           Edema)%>%
  mutate(logViralLoad = log10(ViralLoadmL + 1),
         logAbs = log10(Antibodies)) 
data$Treatment = factor(data$Treatment, levels = trts)


dat.all = data %>% dplyr::select(ID, Treatment, sum.score, logViralLoad, logAbs, PenHDay2, PenHDay3, PenHDay4)
...
png(paste0('res/PCA',paste0(age, sep = '', collapse= ""), '.png'), width = 1200, height = 1000, res = 200)
fig1
dev.off()
...
p1 = forest.plot(res.lsm$contrasts,  -2,8,nudge_x = 2,title = "Aged")
# forest.plot(res.lsm$contrasts, -4e3,1e4,title = "AgedYoung")
png(paste0('res/weightedY1.png'), width = 1800, height = 700, res = 200)
require(gridExtra)
p1
dev.off()
...
ggplot(plot_res, aes(color =Treatment), group = interaction(Treatment)) + 
  ...
  theme_bw() +  xlab('Y = 1/3(logViral + PenH234 + Histopath) - logAbs')+
  scale_color_manual(name = "", values = trt.colorPal) 
  # facet_wrap(~Age, ncol = 1)

# just plot the histograms of objective functions for combined data
# install.packages('jtools')
library(jtools)
res_sum = summ(o.un, confint = TRUE, digits = 3)
plot_res = as.data.frame(res_sum$coeftable) %>% mutate(coef = rownames(plot_res))
colnames(plot_res) = c("mean", "lower", "upper", "t", "p","coef")
ggplot(plot_res,aes(color =coef)) + 
  geom_point( aes(mean,coef)) +
  geom_errorbar(aes(y = coef, xmin = lower, xmax = upper), width =0.5) +
  theme_bw() +  xlab('Estimated Coefficients')  + 
  ylab('')+
...
hist(datanew$logAbs)
hist(datanew$logViral)
hist(datanew$PenH234)
hist(datanew$Histopath)
