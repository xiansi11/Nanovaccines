...
##---------------------Fig.S3(B to E)-----------------------------####
##---------------------Fig.3(C to F)-----------------------------####
filename = "SplenicDC_combined_cytokines_March_Oct"
aa = "Young"

for (tt in c("T cell activation", "Inflammation")){
  cat("\n")
  res_tb = plot_pca(filename, combine = T, age =aa, resp.ex = ex.resp.ls, cytokine.group = tt, no.double = T,max.mice = T)
  cat(paste0(aa,tt))
  print(res_tb)
  if(file.exists(paste0("test.res/PCA_combine_",tt,"_", filename, aa,".csv"))){
